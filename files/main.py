...
def chamar_gemini(client, modelo: str, prompt: str, max_tentativas: int = 4) -> str:
    """Chama o Gemini com retry em caso de rate limit."""
    for tentativa in range(max_tentativas):
        try:
            ...
            if "quota" in msg or "rate" in msg or "429" in msg or "resource_exhausted" in msg:
                ...
                print(f"     ⏳ Limite atingido. Aguardando {espera}s...")
                time.sleep(espera)
                continue
            raise
    raise RuntimeError("Falhou após várias tentativas (limite gratuito do Gemini).")
